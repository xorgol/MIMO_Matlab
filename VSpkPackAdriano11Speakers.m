...
dinfo = dir(fullfile(DirName, '**\*-11speakers.w64'));
...
%
        ...
        if (m==1)
           ...
           [maxPeak, maxIndex] =  max(convRes);
            %Hardcoded max for Kuyus test 4
            maxIndex = 1575680;
...
%     title('Microphone alignment');
    ...
    [DodecEnc,Fs] = audioread('MIMO_Beamformer/Dodec11-Ambix-1st-order.wav');
    hSpk =zeros(4,Spk,2048); %[VSpk x RSpk x N]
    for m=1:1:4
        for c=1:1:Spk
            for s=1:1:2048
